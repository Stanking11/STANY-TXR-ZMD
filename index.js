...
// Download session from MEGA link encoded in config.SESSION_ID
async function downloadSessionData() {
    console.log("Debug: SESSION_ID =", config.SESSION_ID);

    if (!config.SESSION_ID) {
        console.error('❌ Please provide SESSION_ID in the environment variables!');
        return false;
    }

    const sessdata = config.SESSION_ID.split("STANY-TXR-ZMD~")[1];

    if (!sessdata || !sessdata.includes("#")) {
        console.error('❌ Invalid SESSION_ID format. Expected format: JOEL-XMD~fileID#decryptKey');
        return false;
    }
...
│• 🫟𝐕𝐄𝐑𝐒𝐈𝐎𝐍 : 𝟑.𝟎.𝟎
┗───━━━━───━━━━──┉┈⚆`;
    conn.sendMessage(conn.user.id, { image: { url: `https://files.catbox.moe/917teb.jpg` }, caption: up })
  }
  })
...
if (!isReact && senderNumber === botNumber) {
  ...
  app.get("/", (req, res) => {
  res.send("STANY TXR ZMD STARTED ✅");
  });
