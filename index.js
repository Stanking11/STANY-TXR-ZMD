...
async function start() {
    try {
        ...
        console.log(`🤖 STANY-TXR using WA v${version.join('.')}, isLatest: ${isLatest}`);
        
        const Matrix = makeWASocket({
            version,
            logger: pino({ level: 'silent' }),
            printQRInTerminal: useQR,
            browser: ["STANY-TRX-ZMD", "safari", "3.3"],
            auth: state,
            getMessage: async (key) => {
                ...
                return { conversation: "STANY-TRX-ZMD whatsapp user bot" };
            }
        });
...
Matrix.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
        ...
        if (initialConnection) {
            console.log(chalk.green("Connected Successfully STANY-TRX🤍"));
            Matrix.sendMessage(Matrix.user.id, { 
                image: { url: "https://files.catbox.moe/rmxujw.jpeg" }, 
                caption: `*Hello there STANY-TRX User! 👋🏻* 

> Simple, Straightforward, But Loaded With Features 🎊. Meet TRX-ZMD WhatsApp Bot.
